...
nsg_recursive(1, num_paths, node_properties)
plot_graph_google_maps_style()
def integrate_final_json(final_json_path):
    # Load final.json data
    with open(final_json_path, "r") as file:
        final_data = json.load(file)

    # Extract node names from final.json
    final_nodes = {item["Node"] for item in final_data}
    return final_nodes

# Updated plot function with final.json integration
def plot_graph_google_maps_style_with_final(final_json_path):
    # Load final nodes
    final_nodes = integrate_final_json(final_json_path)
    ...
    # Determine node colors and hover texts
    node_colors = ["green" if label in final_nodes else "blue" for label in node_labels]
    hover_texts = [
        f"Node: {label}<br>Color: {'Green (Final)' if label in final_nodes else 'Blue (Not Final)'}"
    ...
    # Layout settings
    fig.update_layout(
        title="Interactive Diverging and Converging Graph with Final Nodes Highlighted",
        xaxis=dict(title="Distance (km)", showgrid=True),
        yaxis=dict(title="Paths", showgrid=True),
        width=6000, height=900,
        dragmode="pan", showlegend=False
...
# Run the NSG algorithm and plot with final.json integration
nsg_recursive(1, num_paths, node_properties)
plot_graph_google_maps_style_with_final("final.json")