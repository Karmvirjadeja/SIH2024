...
export const getWeatherData = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const weatherData = {
      ...
      hourly: {
        ...
        precipitationProbability: hourlyWeather
          ...
          .valuesArray()
          .slice(-1)[0],
        precipitation: hourlyWeather.variables(2).valuesArray().slice(-1)[0],
        snowfall: hourlyWeather.variables(3).valuesArray().slice(-1)[0],
    ...
    // Write combined data to a JSON file in the temp directory
    const tempDir = path.join("temp");
    if (!fs.existsSync(tempDir)) {
      fs.mkdirSync(tempDir, { recursive: true });
    }
    const filePath = path.join(tempDir, "weather_marine_data.json");
    ...
    // Log error and return a proper message
    console.error("Error in getWeatherData:", error);
    return res.status(500).json({
      message: `Failed to fetch weather and marine data: ${error.message}`,
    });
  }
});
